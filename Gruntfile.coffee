...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    copy:
      heroku:
        files: [
          ...
          rename: (dest, src) ->
            path = require("path")
            return path.join(dest, "package.json")  if src is "distpackage.json"
            path.join dest, src
        ]
      ...
      dist:
        files: [
          ...
          cwd: "<%= yeoman.app %>"
          dest: "<%= yeoman.dist %>"
          src: ["*.{ico,png,txt}", ".htaccess", "data/*.json", "images/{,*/}*.{webp,gif}", "styles/fonts/{,*/}*.*", "bower_components/sass-bootstrap/fonts/*.*"]
        ]
    ...
    concurrent:
      server: ["compass", "coffee:dist", "copy:styles"]
  ...
  grunt.registerTask "build", ["clean:dist", "useminPrepare", "concurrent:dist", "autoprefixer", "requirejs", "concat", "cssmin", "uglify", "modernizr", "copy:dist", "copy:heroku", "rev", "usemin"]
